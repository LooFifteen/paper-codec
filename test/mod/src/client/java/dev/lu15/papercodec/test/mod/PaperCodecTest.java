...
public final class PaperCodecTest implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(TestPacket.TYPE, (packet, context) -> {
            LOGGER.info("received packet with block pos {} and string {}", packet.blockPos(), packet.string());
        });

        // send packet on join
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
            ClientPlayNetworking.send(new TestPacket(new BlockPos(0, 0, 0), "Hello, World!"));
        });
    }
